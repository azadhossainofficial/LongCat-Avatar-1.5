...
def load_quantized_dit(checkpoint_dir: str, subfolder: str = "base_model_int8", **kwargs):
    ...
    # Instantiate model on meta device (0 MB RAM allocated)
    ...
    # Materialize any remaining non-quantized meta parameters (Conv3d, Norms) to CPU
    model.to_empty(device="cpu")

    import gc
    # Load quantized state dict shard by shard to minimize peak CPU RAM usage
    ...
    if os.path.exists(index_path):
        ...
        files = sorted([f for f in os.listdir(quantized_dir) if f.endswith(".safetensors") and "index" not in f])
        for f in files:
