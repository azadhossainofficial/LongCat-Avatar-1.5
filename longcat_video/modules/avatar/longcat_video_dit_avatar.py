...
class LongCatAvatarSingleStreamBlock(nn.Module):
    ...
    def forward(
        ...
        # self attn residual addition in native x_dtype (eliminates 1.5GB duplicate FP32 tensors)
        gate_msa_d = gate_msa.to(x_dtype)
        x = x + (gate_msa_d * x_s.view(B, -1, N//T, C)).view(B, -1, C)
        ...
        if not skip_crs_attn:
            ...
            audio_gate_d = audio_gate_mca.to(x_dtype)
            audio_add_x = (audio_gate_d * audio_output_noise.view(B, T-num_cond_latents, -1, C).to(x_dtype)).view(B, -1, C) # [B, N, C]
            if audio_output_cond is not None:
                audio_add_x = torch.cat([audio_output_cond.to(x_dtype), audio_add_x], dim=1).contiguous()
            x = x + audio_add_x

        # ffn with modulation in native x_dtype
        x_m = modulate_fp32(self.mod_norm_ffn, x.view(B, -1, N//T, C), shift_mlp, scale_mlp).view(B, -1, C)
        x_s = self.ffn(x_m)
        gate_mlp_d = gate_mlp.to(x_dtype)
        x = x + (gate_mlp_d * x_s.view(B, -1, N//T, C).to(x_dtype)).view(B, -1, C)
...
class LongCatVideoAvatarTransformer3DModel(
    ...
    def _create_multi_lora_forward(self, module, loras):
        def multi_lora_forward(x, *args, **kwargs):
            ...
            # Chunking along sequence dimension if large to eliminate 6+ GB peak VRAM spikes
            if x.dim() == 3 and x.shape[1] > 4096:
                seq_len = x.shape[1]
                chunk_size = 4096
                out_features = getattr(module, 'out_features', None)
                if out_features is None:
                    # fallback
                    out_features = module.org_forward(x[:, :1], *args, **kwargs).shape[-1]
                org_output = torch.empty((x.shape[0], seq_len, out_features), dtype=weight_dtype, device=x.device)
                for s in range(0, seq_len, chunk_size):
                    e = min(s + chunk_size, seq_len)
                    x_sub = x[:, s:e]
                    sub_org = module.org_forward(x_sub, *args, **kwargs)
                    for lora in loras:
                        if lora.use_lora:
                            scale = float(lora.multiplier * lora.alpha_scale)
                            lx = lora.lora_down(x_sub.to(lora.lora_down.weight.dtype))
                            if hasattr(lora.lora_up, 'blocks'):
                                xs = torch.chunk(lx, len(lora.lora_up.blocks), dim=-1)
                                sub_dim = sub_org.shape[-1] // len(lora.lora_up.blocks)
                                for bi, blk in enumerate(lora.lora_up.blocks):
                                    blk_out = blk(xs[bi])
                                    sub_org[..., bi*sub_dim:(bi+1)*sub_dim].add_(blk_out.to(weight_dtype), alpha=scale)
                                    del blk_out
                            else:
                                blk_out = lora.lora_up(lx)
                                sub_org.add_(blk_out.to(weight_dtype), alpha=scale)
                                del blk_out
                            del lx
                    org_output[:, s:e] = sub_org
                    del sub_org
                return org_output
            else:
                ...
                for lora in loras:
                    if lora.use_lora:
                        ...
                        if hasattr(lora.lora_up, 'blocks'):
                            xs = torch.chunk(lx, len(lora.lora_up.blocks), dim=-1)
                            sub_dim = org_output.shape[-1] // len(lora.lora_up.blocks)
                            for bi, blk in enumerate(lora.lora_up.blocks):
                                blk_out = blk(xs[bi])
                                org_output[..., bi*sub_dim:(bi+1)*sub_dim].add_(blk_out.to(weight_dtype), alpha=scale)
                                del blk_out
                        else:
                            blk_out = lora.lora_up(lx)
                            org_output.add_(blk_out.to(weight_dtype), alpha=scale)
                            del blk_out
                        del lx
                return org_output
    ...
    def forward(
        ...
        if encoder_attention_mask is not None:
            ...
            encoder_hidden_states = encoder_hidden_states.masked_select(encoder_attention_mask.unsqueeze(-1) != 0).view(1, -1, hidden_states.shape[-1]) # [1, N_valid_tokens, C]
            y_seqlens = encoder_attention_mask.sum(dim=1).tolist() # [B]
        else:
            if encoder_hidden_states.dim() == 4:
                seq_len = encoder_hidden_states.shape[2]
                encoder_hidden_states = encoder_hidden_states.squeeze(1)
            else:
                seq_len = encoder_hidden_states.shape[1]
            y_seqlens = [seq_len] * encoder_hidden_states.shape[0]
