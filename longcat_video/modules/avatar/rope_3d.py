...
class RotaryPositionalEmbedding(nn.Module):
    ...
    def forward(self, q, k, grid_size, frame_index=None, num_ref_latents=None):
        ...
        orig_dtype = q.dtype
        cos = freqs_cis.cos().to(dtype=orig_dtype, device=q.device)
        sin = freqs_cis.sin().to(dtype=orig_dtype, device=q.device)
        ...
        def _apply_rope(x):
            ...
            x1 = x_pairs[..., 0]
            x2 = x_pairs[..., 1]
            cos_p = cos.view(*cos.shape[:-1], -1, 2)[..., 0]
            sin_p = sin.view(*sin.shape[:-1], -1, 2)[..., 0]
            out = torch.empty_like(x_pairs)
            out[..., 0] = x1 * cos_p - x2 * sin_p
            out[..., 1] = x1 * sin_p + x2 * cos_p
