...
class FeedForwardSwiGLU(nn.Module):
    ...
    def forward(self, x):
        if x.dim() == 3 and x.shape[1] > 4096:
            seq_len = x.shape[1]
            chunk_size = 4096
            out = torch.empty((x.shape[0], seq_len, self.dim), dtype=x.dtype, device=x.device)
            for s in range(0, seq_len, chunk_size):
                e = min(s + chunk_size, seq_len)
                x_c = x[:, s:e]
                out[:, s:e] = self.w2(F.silu(self.w1(x_c)) * self.w3(x_c))
            return out
...
class RMSNorm_FP32(torch.nn.Module):
    ...
    def forward(self, x):
        # In-place normalized output without allocating new tensor
        var = x.pow(2).mean(-1, keepdim=True).float()
        scale = torch.rsqrt(var + self.eps).type_as(x) * self.weight
        return x.mul_(scale)
...
class LayerNorm_FP32(nn.LayerNorm):
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        # Use native layer_norm which handles mixed precision internally without copying full tensor to float32
        out = F.layer_norm(
            inputs, 
...
def modulate_fp32(norm_func, x, shift, scale):
    ...
    if dtype in (torch.bfloat16, torch.float16):
        # Native layer norm on Ampere/Ada/Blackwell handles mixed precision natively with internal FP32 accumulation
        # avoiding duplicate 1.12GB FP32 tensor allocations per block
        x = norm_func(x)
        x = x * (scale.to(dtype) + 1) + shift.to(dtype)
        return x
    else:
