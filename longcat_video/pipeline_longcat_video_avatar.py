...
class LongCatVideoAvatarPipeline:
    ...
    def _get_t5_prompt_embeds(
        ...
        # Dynamic Zero-OOM VRAM Staging:
        # Run text_encoder on GPU for 0.5s ultra-fast prompt encoding.
        # If DiT is already on GPU taking VRAM, temporarily offload DiT to CPU for 0.5s to guarantee 0 OOM.
        dit_temp_offloaded = False
        prompt_embeds = None
        if torch.cuda.is_available() and target_device != "cpu":
            try:
                dev_idx = torch.device(target_device).index or 0
                free_bytes = torch.cuda.mem_get_info(dev_idx)[0]
                free_gb = free_bytes / (1024 ** 3)
                if free_gb < 11.0 and self.dit is not None:
                    # DiT is in VRAM; offload to CPU temporarily for ~0.4s
                    try:
                        self.dit.to("cpu")
                        dit_temp_offloaded = True
                        torch.cuda.empty_cache()
                    except Exception:
                        dit_temp_offloaded = False
                ...
                with torch.no_grad():
                    prompt_embeds = self.text_encoder(text_input_ids.to(target_device), mask.to(target_device)).last_hidden_state
                self.text_encoder.to("cpu")
                torch.cuda.empty_cache()

                if dit_temp_offloaded and self.dit is not None:
    ...
    def encode_prompt(
        ...
        r"""
        Encodes the prompt into text encoder hidden states with automatic persistent caching.
        """
        cache_key = (
            str(prompt),
            str(negative_prompt),
            bool(do_classifier_free_guidance),
            int(num_videos_per_prompt),
            int(max_sequence_length),
            str(device or self.device),
            str(dtype or self.text_encoder.dtype),
        )
        if hasattr(self, "_prompt_embeds_cache") and cache_key in self._prompt_embeds_cache:
            p_emb, p_mask, n_emb, n_mask = self._prompt_embeds_cache[cache_key]
            return p_emb.clone(), p_mask.clone(), (n_emb.clone() if n_emb is not None else None), (n_mask.clone() if n_mask is not None else None)
        ...
        if not hasattr(self, "_prompt_embeds_cache"):
            self._prompt_embeds_cache = {}
        self._prompt_embeds_cache[cache_key] = (
            prompt_embeds.clone(),
            prompt_attention_mask.clone(),
            (negative_prompt_embeds.clone() if negative_prompt_embeds is not None else None),
            (negative_prompt_attention_mask.clone() if negative_prompt_attention_mask is not None else None),
        )
    ...
    def get_timesteps_sigmas(self, sampling_steps: int, use_distill: bool=False):
        if use_distill:            
            if self.model_type == "avatar-v1.5":
                ...
                raw_sigmas = torch.flip(torch.linspace(0, 1, self.num_timesteps), [0])
                distill_sigmas = torch.flip(raw_sigmas[distill_indices], [0]).float()
                
                if sampling_steps == self.num_distill_sample_steps:
                    sigmas = distill_sigmas
                elif sampling_steps < self.num_distill_sample_steps:
                    indices = np.linspace(0, len(distill_sigmas) - 1, num=sampling_steps).round().astype(np.int64)
                    sigmas = distill_sigmas[indices]
                else:
                    sigmas = torch.linspace(distill_sigmas[0].item(), distill_sigmas[-1].item(), sampling_steps)
            else:
                distill_indices = torch.arange(1, self.num_distill_sample_steps + 1, dtype=torch.float32)
                distill_indices = (distill_indices * (self.num_timesteps // self.num_distill_sample_steps)).round().long()
    ...
    def get_audio_embedding_whisper(self, speech_array, fps=25, device='cpu', sample_rate=16000):
        ...
        with torch.no_grad():
            ...
            audio_prompts = torch.cat(enc_chunks, dim=1)         # [1, T_enc_total, n_layers, D] (on CPU)
            audio_prompts = audio_prompts[:, :video_length * 2]  # 截取有效帧

            # ---- 按层分组 + 插值到目标帧数 (CPU based) ----
    ...
    def generate_at2v(
        ...
        with tqdm(total=len(timesteps), desc="Denoising") as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i == len(timesteps) - 1 or (i + 1) % self.scheduler.order == 0:
                    progress_bar.update()
                    step_latency = max(0.1, time.time() - step_start_time)
                    step_start_time = time.time()
                    if context_parallel_util.get_cp_rank() == 0:
                        pct = int(((i + 1) / len(timesteps)) * 100)
                        bar_filled = int(pct / 10)
                        print(f"Denoising: {pct}%|{'█'*bar_filled}{' '*(10-bar_filled)}| {i+1}/{len(timesteps)} [{step_latency:.1f}s/it] [Step {i+1}/{len(timesteps)}]", flush=True)
        ...
        latents_to_decode = latents.to(self.vae.dtype)
        latents_to_decode = self.denormalize_latents(latents_to_decode)
        output_video = self.vae.decode(latents_to_decode, return_dict=False)[0]
    ...
    def generate_ai2v(
        ...
        Args:
            ...
            negative_prompt (`str or List[str]`, *optional*):
                Negative prompt(s) for content exclusion. If not provided, uses empty string.
            resolution (`Literal["520p", "580p", "600p", "700p", "720p", "480p"]`, *optional*, defaults to "520p"):
    ...
    def generate_avc(
        ...
        # 5. Prepare latent variables
        num_cond_latents = 1 + (num_cond_frames - 1) // self.vae_factor_temporal if hasattr(self, "vae_factor_temporal") else (1 + (num_cond_frames - 1) // self.vae_scale_factor_temporal)
        if video_latent is not None:
            cond_videos_latents = video_latent[:, :, -num_cond_latents:].to(device=device, dtype=dit_dtype)
        else:
            ...
            cond_videos_latents = self.normalize_latents(cond_videos_latents).to(device=device, dtype=dit_dtype)
            cond_videos_latents = torch.nan_to_num(cond_videos_latents, nan=0.0, posinf=5.0, neginf=-5.0)
            cond_videos_latents = torch.clamp(cond_videos_latents, -6.0, 6.0)
            if self.vae is not None:
                self.vae.to("cpu")
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
        ...
        latents_to_decode = latents.to(self.vae.dtype)
        latents_to_decode = self.denormalize_latents(latents_to_decode)
        latents_to_decode = torch.nan_to_num(latents_to_decode, nan=0.0, posinf=35.0, neginf=-35.0)
        output_video = self.vae.decode(latents_to_decode, return_dict=False)[0]
        output_video = self.video_processor.postprocess_video(output_video)
        if self.vae is not None:
            self.vae.to("cpu")
    ...
    def to(self, device: str | torch.device):
        ...
        # text_encoder is kept on CPU and dynamically staged during prompt encoding to save 9.5 GB VRAM
        # vae is kept on CPU and dynamically staged during prepare_latents/decode to save 4.5 GB VRAM
        if self.vae is not None:
